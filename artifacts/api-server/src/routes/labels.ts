...
router.get("/labels", async (_req, res): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: `Eroare la încărcarea etichetelor: ${message}` });
  }
});
...
router.get("/labels/:id", async (req, res): Promise<void> => {
  try {
    ...
    console.error("[Labels] Error fetching label:", error);
    const message = error instanceof Error ? error.message : "Eroare necunoscută";
    res.status(500).json({ error: `Eroare la încărcarea etichetei: ${message}` });
  }
});
